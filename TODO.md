...
- **Tap-to-interrupt**: clicking/tapping the face while `THINKING`/`SPEAKING` sends `{"type":"interrupt"}` back over the same WebSocket (the one client→server message the IPC carries — everything else is Orchestrator→UI). `conversation.Service.Interrupt` cancels the in-flight turn's `context.Context`, aborting the LLM request and killing the `piper`/playback subprocess immediately rather than just suppressing future sentences. Ported from be-more-agent's keyboard-interrupt (space bar), adapted to a tap since a browser tab has no reliable keyboard focus. Verified with a fake slow-LLM harness: interrupted turn returned in ~300ms instead of blocking for its full 5s mock duration.
