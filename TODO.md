...
- `internal/adapter/wakeword` — `NoOp` stub (never fires — real engine still unselected) and `Always` (dev/test-only, never wired into the shipped CLI).
...
1. **Wake-word engine selection** — currently a stub that never fires; nothing will be transcribed in `--audio` mode until a real engine (Porcupine, openWakeWord, ...) is chosen and wired in. Blocks all real end-to-end audio testing.
2. **Piper TTS integration** — still a stub that logs instead of speaking. Piper has no official server mode like whisper.cpp/llama.cpp, so needs its own CGo-vs-subprocess decision (likely CLI-per-utterance subprocess, or ONNX Runtime CGo if latency requires it).
3. **Claude CLI / OpenCode agent sandbox enforcement** — policy is specified (deps-only network, scoped filesystem writes) but not the enforcement primitive (Linux namespaces? restricted user + seccomp? container?). `StubAgent` always fails until this is decided.
4. **Auth/key storage for Claude CLI / OpenCode** — `StubAuth` always returns unauthorized. Needs OS keyring or `0600`-permission config file, per RFC Security Implications.
5. **`EXECUTE_AGENT` timeout** — no concrete value wired in yet (RFC suggests ~120s) plus child-facing fallback wording.
6. **Concurrent `EXECUTE_AGENT` request handling** — behavior undefined if a second request arrives mid-generation (queue vs. reject).
7. **UI layer** — `StubUI` only logs; no real webview/Neutralinojs window, no mouth-sync animation, no floating-overlay mode.
8. **Real-hardware benchmarking (Phase 1 blocker)** — `<2.0s` voice latency target (endpointing + STT + LLM first token) unverified on actual i5-2510M under `-t 2`; VAD timing constants (700ms silence / 300ms pre-roll / 20s cap) unvalidated against real child speech patterns.
9. **Energy VAD robustness** — pure RMS-threshold classifier is more sensitive to background noise than a spectral classifier; revisit WebRTC VAD (small CGo lib) after real-room Phase 1 testing.
10. **`models.yaml` checksums** — `sha256` fields ship blank; must be filled in (or accept manual-copy fallback) before `naira models download` will auto-fetch anything.
11. **Screen-time `[SLEEPY]` behavior** — logging/threshold-check logic exists in the state service but isn't wired into the orchestrator loop or UI yet.
12. **Claude CLI cost/rate-limit exposure** — no budget or rate-limiting specified; a chatty child could trigger many generation requests.
13. **Subsystem-failure UX** — process supervisor marks a subsystem permanently unhealthy after exhausting restart attempts, but no UI-facing degraded-mode expression is wired to that yet.
14. **RFC.md header placeholders** — `Owner`/`Approver` still `_TBD_`.
