...
- **Audio capture, VAD, endpointing** (no CGo): `internal/adapter/audio` (arecord/parec subprocess → fixed 20ms PCM16 frames + in-memory WAV encoding), `internal/adapter/vad` (RMS-threshold + adaptive-noise-floor energy VAD), `internal/usecase/listening` (wake-word-gated capture, ~300ms pre-roll seed, 700ms silence cutoff, 20s max-utterance safety cap). **VAD-gated wake-word inference**: idle-state frames only get passed to `Wake.Detect` (an HTTP round-trip to `openwakeword_server.py`) when `VAD.IsSpeech` says the frame has energy — silent frames are skipped entirely, saving CPU/latency on the 2-core target. As a side effect, the noise floor now calibrates continuously from startup instead of only once `LISTENING` begins.
- **Mic sample-rate fallback**: `internal/adapter/audio.Resample` (pure-Go linear interpolation, no CGo/DSP dependency) + `MicCapture.CaptureRate`/`--mic-rate`. Default (`0`) requests 16kHz directly from `arecord`, byte-identical to before this existed. If set, requests that rate instead and resamples every chunk down to 16kHz in Go, re-slicing the variable-length resampled output into the fixed `domain.AudioFrameBytes` frames the rest of the pipeline assumes (carrying leftover bytes across reads). **Scoped down from true auto-detection**: this is a manual flag, not runtime ALSA device-capability probing — that needs the actual target hardware in hand to validate, which wasn't available here. Verified with synthetic data: resampler output length/amplitude checked against a synthetic 44.1kHz tone; frame re-slicing checked end-to-end via a real subprocess (`bash -c 'cat <file>'` standing in for `arecord`) streaming 1s of 44.1kHz PCM, producing exactly 50 correctly-sized 16kHz frames.
...
### Face UI
- `internal/adapter/ui.Server` — loopback HTTP+WebSocket server (`--ui-port`, default 8090) implementing `domain.UIPublisher`; broadcasts `state_change`/`mouth_amplitude`/`window_mode`/`agent_status`/`speak_chunk` (RFC.md#apis Internal IPC) to every connected client, no CGo. `speak_chunk` (subtitle/caption) renders in a caption box over the face (`#caption`, hidden on `IDLE`) — not cached for replay like `state_change`/`window_mode`, since a caption is only meaningful in the moment it's spoken.
...
- **Tap-to-interrupt / push-to-talk (one gesture, state-gated)**: clicking/tapping the face sends `{"type":"interrupt"}` while `THINKING`/`SPEAKING`, or `{"type":"ptt"}` while `IDLE` — the two client→server messages the IPC carries (everything else is Orchestrator→UI). `conversation.Service.Interrupt` cancels the in-flight turn's `context.Context`, aborting the LLM request and killing the `piper`/playback subprocess immediately rather than just suppressing future sentences (verified with a fake slow-LLM harness: interrupted turn returned in ~300ms instead of blocking for its full 5s mock duration). `listening.Service.PTT` (optional channel, nil-safe) lets a tap start an utterance exactly like a wake-word detection would, bypassing `Wake.Detect` entirely — a manual fallback for when the wake word is unreliable on real hardware (verified with a fake-Wake/VAD/STT harness: transcript produced, `Wake.Detect` never called). Both ported from be-more-agent's keyboard-interrupt/PTT (Enter/space keys), adapted to taps since a browser tab has no reliable keyboard focus.
