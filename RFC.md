...
| `interrupt` | UI → Orchestrator | `{ "type": "interrupt" }` | **Addition, not in original spec.** Tap/click on the face while `THINKING`/`SPEAKING` cancels the in-flight turn (`conversation.Service.Interrupt`) — cancels the turn's `context.Context`, which aborts the LLM request and kills the `piper`/playback subprocess immediately rather than only suppressing future sentences. The only client→server direction this channel carries; every other message flows Orchestrator → UI. Ported from the reference project's keyboard-interrupt precedent ([brenpoly/be-more-agent](https://github.com/brenpoly/be-more-agent)), adapted from a keypress to a tap since a browser tab has no reliable keyboard focus. |
