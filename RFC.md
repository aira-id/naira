...
| `arecord`/`parec` (ALSA/PulseAudio CLI, subprocess) | Continuous mic capture → raw PCM16 mono | Spawned once at startup, stdout piped as a continuous frame stream; no CGo audio library needed. If the mic doesn't cleanly support 16kHz, `--mic-rate` requests its native rate instead and `internal/adapter/audio.Resample` (pure-Go linear interpolation) converts down to 16kHz in-process — manual fallback, not runtime auto-detection (see [§5 Concerns](#5-concerns-questions-or-known-limitations)) |
...
| `interrupt` | UI → Orchestrator | `{ "type": "interrupt" }` | **Addition, not in original spec.** Tap/click on the face while `THINKING`/`SPEAKING` cancels the in-flight turn (`conversation.Service.Interrupt`) — cancels the turn's `context.Context`, which aborts the LLM request and kills the `piper`/playback subprocess immediately rather than only suppressing future sentences. Ported from the reference project's keyboard-interrupt precedent ([brenpoly/be-more-agent](https://github.com/brenpoly/be-more-agent)), adapted from a keypress to a tap since a browser tab has no reliable keyboard focus. |
| `ptt` | UI → Orchestrator | `{ "type": "ptt" }` | **Addition, not in original spec.** Same tap gesture, but while `IDLE`: a manual push-to-talk fallback (`listening.Service.PTT`, an optional nil-safe channel) that starts an utterance exactly like a wake-word detection would, bypassing `Wake.Detect` entirely — useful when the wake word is unreliable on real hardware. Ported from be-more-agent's Enter-key PTT toggle, adapted to a tap for the same reason as `interrupt` above. These two are the only client→server messages the IPC carries; every other message flows Orchestrator → UI. |
...
- **Mic sample-rate handling is a manual flag, not auto-detection.** `--mic-rate` + `internal/adapter/audio.Resample` let an operator work around a mic that doesn't support 16kHz cleanly, but there's no runtime ALSA device-capability probing (unlike the reference project's `sounddevice`-based auto-detect+resample, which needed it to support arbitrary consumer USB mic/speaker combos on a Pi). Naira targets one fixed device, so this was scoped down deliberately — revisit if real hardware testing turns up a mic that needs it and manual configuration proves annoying.
