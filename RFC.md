...
| `speak_chunk` | Orchestrator → UI | `{ "text": "<sentence>", "seq": <int> }` | Optional subtitle/caption display alongside TTS — spec'd here, not yet implemented (see [§5 Concerns](#5-concerns-questions-or-known-limitations)) |
...
- **`speak_chunk` (subtitle/caption display) spec'd but not implemented.** The IPC table has carried this message type since the original design, but no code path emits it yet — `internal/adapter/ui/static`'s face client has no caption UI either. Cheap addition if wanted: extend `domain.UIPublisher` and call it alongside `TTS.Speak` in `internal/usecase/conversation.Service`.
